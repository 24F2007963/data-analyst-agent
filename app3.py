...
LLM_API_KEY = "eyJhbGciOiJIUzI1NiJ9.eyJlbWFpbCI6InNtcml0aS50dXJhbkBnbWFpbC5jb20ifQ.9pqId92evbHHNgH8I5FWqTHYqdMSmo-bM2TnLi2XEgg"  # If required by your proxy
...
Use the question given. Analyze and find the appropriate url and other data. Provide additional data that you find that can be used to answer the questions asked.

Schema (must validate):
{
  "AdditionalData": {
  "data":"data to answer the given questions"
  },
  "type": "object",
  "properties": {
    "inputs": {
      ...
      "items": {
        ...
        "properties": {
          "name": {"type":"string"},
          "source": {"type":"string", "enum": ["html","csv","json","inline", "parquet_s3"]},
    ...
    "answer": {
      "type":"object",
      "properties": {
        "type": {"type":"string","enum":["text_summary","basic_stats","regression_summary","none"]},
...
When the question involves large datasets stored as parquet files in S3, such as Indian High Court judgments dataset, plan to:

- Use DuckDB to query parquet files directly on S3 to retrieve aggregates or filtered data.
- Transform regression filter as dict, not string
- Do not return empty url for parquet_s3 files. Analyze and provide url 
- Include SQL queries in your plan with table paths like s3://bucket/path/*.parquet.
- If the question involves date regression or delays, plan a regression analysis step.
- For questions asking for plots, plan plotting tasks to generate scatterplots or line charts.
- Keep the plan minimal with up to 3 steps.
Minimize transforms. Prefer simple operations. Return JSON only."""

# Simplified formatter for speed
OUTPUT_FORMATTER_SYSTEM = """ Analyze the given questions on your own and return the answers in ONE string only containing answers.
"""

OUTPUT_FORMATTER_SYSTEM2 = """Analyze the results data and if results suggest that data is not available. Analyze the given questions on your own and return the answers in ONE string only containing answers.
Format the result concisely. Return ONE string only. Use categorical answer only if question suggestes.
no need for answer number. make all the calculations twice to make sure the answer is correct.
...
def execute_duckdb_query(query: str) -> list:
    con = duckdb.connect(database=':memory:')
    con.execute("INSTALL httpfs;")
    con.execute("LOAD httpfs;")
    con.execute("INSTALL parquet;")
    con.execute("LOAD parquet;")
    # You can set your region if needed:
...
def regression_and_plot(data: list, x_col: str, y_col: str):
...
def apply_transform(df_map: Dict[str, Any], t: Transform, logs: List[str]) -> None:
    ...
    try:
        if t.op == "duckdb_query":
            query = task.get("query")
            logs.append(f"Running DuckDB query: {query[:50]}...")
            try:
                result = execute_duckdb_query(query)
                duckdb_results.append(result)
                answers[task.get("question", "duckdb_query_result")] = result
            except Exception as e:
                logs.append(f"Error running DuckDB query: {e}")
                answers[task.get("question", "duckdb_query_result")] = f"Error: {e}"
        ...
        elif t.op == "duckdb_query":
            # Run SQL query on registered dataframes in DuckDB
            print('received duckdb query')
            query = t.args.get("query")
            if not query:
                logs.append(f"[transform] duckdb_query missing query string")
                return
            
            con = duckdb.connect()
            # Register all available dataframes as DuckDB views
            for name, df in df_map.items():
                con.register(name, df)
            
            # Execute query, fetch result as pandas df
            df_result = con.execute(query).fetchdf()
            df_map[t.target] = df_result
            logs.append(f"[transform] duckdb_query on {t.target} executed, rows={len(df_result)}")
...
def run_analysis_spec(spec: AnalysisSpec) -> Dict[str, Any]:
    ...
    # tasks = getattr(spec.parameters, "tasks", None) or []
    # for task in tasks:
    #     ttype = task.get("type")
    #     if ttype == "duckdb_query":
    #         query = task.get("query")
    #         logs.append(f"Running DuckDB query: {query[:50]}...")
    #         try:
    #             result = execute_duckdb_query(query)
    ...
    #         try:
    #             last_result = duckdb_results[-1] if duckdb_results else []
    #             slope, plot_uri = regression_and_plot(
    #                 data=last_result,
    #                 x_col=task.get("x_column"),
    #                 y_col=task.get("y_column")
    #             )
    #             regression_results.append(slope)
    #             question = task.get("question", "regression_result")
    #             answers[question] = slope
    #             plot_question = task.get("plot_question")
    #             if plot_question:
    #                 answers[plot_question] = plot_uri
    #             images.append(plot_uri)
    #             logs.append(f"Performed regression with slope={slope}")
    #         except Exception as e:
    #             logs.append(f"Regression error: {e}")
    #             answers[task.get("question", "regression_result")] = f"Error: {e}"
...
async def analyze_question_async(question: str, debug: int = 0):
    ...
    try:
        ...
        print('final_structured1 : ', final_structured)
    ...
    try:
        print('final_structured2 : ', final_structured)
